...
// List lists all CStorPoolInstances in the indexer.
func (s *cStorPoolInstanceLister) List(selector labels.Selector) (ret []*v1.CStorPoolInstance, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CStorPoolInstance))
	})
	return ret, err
}
...
// List lists all CStorPoolInstances in the indexer for a given namespace.
func (s cStorPoolInstanceNamespaceLister) List(selector labels.Selector) (ret []*v1.CStorPoolInstance, err error) {
...
// Get retrieves the CStorPoolInstance from the indexer for a given namespace and name.
func (s cStorPoolInstanceNamespaceLister) Get(name string) (*v1.CStorPoolInstance, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cstorpoolinstance"), name)
	}
	return obj.(*v1.CStorPoolInstance), nil
}
