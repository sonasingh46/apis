...
type CStorVolumeLister interface {
	...
	List(selector labels.Selector) (ret []*v1.CStorVolume, err error)
...
// cStorVolumeLister implements the CStorVolumeLister interface.
type cStorVolumeLister struct {
...
func NewCStorVolumeLister(indexer cache.Indexer) CStorVolumeLister {
	return &cStorVolumeLister{indexer: indexer}
}

// List lists all CStorVolumes in the indexer.
func (s *cStorVolumeLister) List(selector labels.Selector) (ret []*v1.CStorVolume, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.CStorVolume))
	})
	return ret, err
}
...
type CStorVolumeNamespaceLister interface {
	...
	Get(name string) (*v1.CStorVolume, error)
...
// List lists all CStorVolumes in the indexer for a given namespace.
func (s cStorVolumeNamespaceLister) List(selector labels.Selector) (ret []*v1.CStorVolume, err error) {
...
// Get retrieves the CStorVolume from the indexer for a given namespace and name.
func (s cStorVolumeNamespaceLister) Get(name string) (*v1.CStorVolume, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("cstorvolume"), name)
	}
	return obj.(*v1.CStorVolume), nil
}
